function main () {
	...
	var trustedCIRegex = /^(OpenStack CI|Jenkins|jenkins-ci)$/;
	// this regex matches the pipeline markup
	var pipelineNameRegex = /<\/p><p>(.*) build/;
	// The url to full status information on running jobs
	var zuulStatusURL = 'http://integration.wikimedia.org/zuul';
	// The json URL to check for running jobs
	var zuulStatusJSON = 'https://integration.wikimedia.org/zuul/status.json';
	...
	*/
	...
	var ci_parse_results = function($panel) {
		var result_list = [];
		var test_results = $panel.find("li.ci_comment_test");
		var pipeline = null;
		if (test_results !== null) {
			test_results.each(function(i, li) {
				var result = {};
				result["name"] = jQ(li).find("span.ci_comment_test_name").find("a")[0].innerHTML;
				result["link"] = jQ(li).find("span.ci_comment_test_name").find("a")[0];
				result["result"] = jQ(li).find("span.ci_comment_test_result")[0];
	...
	var ci_parse_comments = function() {
		var comments = [];
		jQ("p").each(function() {
			var match = psRegex.exec(jQ(this).html());
			if (match !== null) {
				var psnum = parseInt(match[2]);
				var top = jQ(this).parent().parent().parent();
				// old change screen
				var name = top.attr("name");
				if (!name) {
					// new change screen
					name = jQ(this).parent().prev().children()[0].innerHTML;
				}
				var comment = {};
				comment.name = name;
				...
				if (date_cell.attr("title")) {
					...
					// new change screen
					comment.date = jQ(this).parent().prev().children()[2].innerHTML;
				}
				var comment_panel = jQ(this).parent();
				comment.psnum = psnum;
	...
	var ci_prepare_results_table = function() {
		...
		var table = jQ("table.test_result_table")[0];
		if (!table) {
			table = document.createElement("table");
			jQ(table).addClass("test_result_table");
			jQ(table).addClass("infoTable").css({"margin-top":"1em", "margin-bottom":"1em"});

			var approval_table = jQ("div.approvalTable");
			if (approval_table.length) {
				var outer_table = document.createElement("table");
				jQ(outer_table).insertBefore(approval_table);
				var outer_table_row = document.createElement("tr");
				jQ(outer_table).append(outer_table_row);
				var td = document.createElement("td");
				jQ(outer_table_row).append(td);
				jQ(td).css({"vertical-align":"top"});
				jQ(td).append(approval_table);
				td = document.createElement("td");
				jQ(outer_table_row).append(td);
				jQ(td).css({"vertical-align":"top"});
				jQ(td).append(table);
			} else {
				var big_table_row = jQ("div.screen>div>div>table>tbody>tr");
				var td = jQ(big_table_row).children()[1];
				jQ(td).append(table);
			}
		} else {
			jQ(table).empty();
	...
	var ci_display_results = function(comments) {
		...
		if (ci_is_merge_conflict(comments)) {
			var mc_header = jQ("<tr>").append(jQ('<td class="merge_conflict" colpsan="2">Patch in Merge Conflict</td>'));
			mc_header.css('width', '400');
			mc_header.css('font-weight', 'bold');
			mc_header.css('color', 'red');
			mc_header.css('padding-left', '2em');
			jQ(table).append(mc_header);
		...
		for (var i = 0; i < pipelines.length; i++) {
			...
			var header = jQ("<tr>").append(jQ('<td class="header">' + comment.name + " " + pipeline_name + rechecks + '</td>'));
			header.append('<td class="header ci_date">' + comment.date + '</td>');
			jQ(table).append(header);
			for (var j = 0; j < comment.results.length; j++) {
				var result = comment.results[j];
				var tr = jQ("<tr>");
				tr.append(jQ("<td>").append(jQ(result["link"]).clone()));
				tr.append(jQ("<td>").append(jQ(result["result"]).clone()));
				jQ(table).append(tr);
			}
		}
	};
	...
	var ci_toggle_visibility = function(comments, showOrHide) {
		...
		jQ.each(comments, function(i, comment) {
			if (comment.is_ci) {
				jQ(comment.ref).toggle(showOrHide);
			}
		});
	};
	...
	var ci_page_loaded = function() {
		if (hashRegex.test(window.location.hash)) {
			...
			// restore toggle ci button visibility (changed to none)
			jQ('#toggleci').show();
	...
	var ci_zuul_display_status = function(status) {
		var zuul_table = jQ("table.zuul_result_table")[0];
		if (!zuul_table) {
			var test_results = jQ("table.test_result_table")[0];
			zuul_table = document.createElement("table");
			jQ(zuul_table).addClass("zuul_result_table");
			jQ(zuul_table).addClass("infoTable").css({"margin-bottom":"1em"});
			if (test_results) {
				jQ(test_results).prepend(zuul_table);
			}
		}
		jQ(zuul_table).empty();
		jQ(zuul_table).show();
		jQ(zuul_table).append("<tr><td class='header'>Change currently being tested (<a href='" + zuulStatusURL + "'>full status</a>)</td></tr>");
    ...
    var observer = new MutationObserver(function(mutations, observer) {
        var span = jQ("span.rpcStatus");
        jQ.each(mutations, function(i, mutation) {
...
// Add jQuery to the head tag, and load Toggle CI after it finished loading.
...
// if the page is ready, add jQuery and start Toggle CI
